...
#     pct = int((gap / average_likes) * 100) if average_likes > 0 else 0
#     return f"""### ❌ Post Failure Analysis: {post.get('date', 'N/A')}
# * 📊 **The Performance Gap:** Falls {gap:,} likes below the baseline average of {int(average_likes):,} likes.
# * 📉 **Why it is Failing:** Lacked an open-ended comment trigger or visual hook, resulting in immediate retention drop-offs and minimal community interaction.
# * 💡 **Reach Recovery Suggestion:** Next time, structure your post with an alternative high-reach visual layout and start the caption with a direct, open-ended question targeting a specific customer pain point."""
import os
import time
import json
